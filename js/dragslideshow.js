(function (window) {
  ...
  DragSlideshow.prototype._initEvents = function () {
    ...
    this.slides.forEach(function (slide) {
      ...
      // reveal content
      slide
        .querySelector("button.content-switch")
  ...
  DragSlideshow.prototype._getContentPage = function (slide) {
    return this.pages.querySelector(
      'div.content[data-content = "' + slide.getAttribute("data-content") + '"]'
    );
  };
  ...
  DragSlideshow.prototype._toggleContent = function (slide) {
    ...
    var main_switch1 = $(".switch1");
    var main_switch2 = $(".switch2");
    ...
    if (this.isContent) {
      ...
      main_switch1.removeClass("go_main");
      main_switch2.removeClass("go_main");
      main_switch1.addClass("main_switch1");
      main_switch2.addClass("main_switch2");
    } else {
      ...
      main_switch1.removeClass("main_switch1");
      main_switch2.removeClass("main_switch2");
      main_switch1.addClass("go_main");
      main_switch2.addClass("go_main");
    }

    var self = this,
      onEndTransitionFn = function (ev) {
        if (support.transitions) {
          if (ev.propertyName.indexOf("transform") === -1 || ev.target !== this)
            return;
  ...
  DragSlideshow.prototype.toggle = function () {
    ...
    this.imgDragger.style.WebkitTransform =
      "perspective(" +
      this.options.perspective +
      "px) translate3d( -50%, -50%, " +
      zAxisVal +
      "px )";
    this.imgDragger.style.transform =
      "perspective(" +
      this.options.perspective +
      "px) translate3d( -50%, -50%, " +
      zAxisVal +
    ...
    var onEndTransitionFn = function (ev) {
      ...
      classie.remove(
        this,
        self.isFullscreen ? "img-dragger-large" : "img-dragger-small"
      );
      classie.add(
        this,
        self.isFullscreen ? "img-dragger-small" : "img-dragger-large"
      );
      ...
      this.style.width = self.isFullscreen
        ? self.options.slideshowRatio * 100 + "%"
        : "100%";
      this.style.height = self.isFullscreen
        ? self.options.slideshowRatio * 100 + "%"
        : "100%";
